...
L_IK_bone = ob.pose.bones[ "Hand_IK_L" ]
L_IK_bone.rotation_mode = 'XYZ'

R_IK_bone = ob.pose.bones[ "Hand_IK_R" ]
R_IK_bone.rotation_mode = 'XYZ'
...
#        imgCv2 = cv.imdecode(imgNp2, -1)
        ...
        if results1.pose_world_landmarks and results2.pose_world_landmarks:
            ...
            LLA_X = ( ( (LLA_lm1.x + LLA_lm2.x) / 2 )*100 )
            LLA_Y = ( ( (LLA_lm1.y + LLA_lm2.y) / 2 )*100 )
            LLA_Z = ( ( (LLA_lm1.z + LLA_lm2.z) / 2 )*100 )
            ...
            RLA_X = ( (RLA_lm1.x + RLA_lm2.x) / 2 ) * 100
            RLA_Y = ( (RLA_lm1.y + RLA_lm2.y) / 2 ) * 100
            RLA_Z = ( (RLA_lm1.z + RLA_lm2.z) / 2 ) * 100
            ...
            L_IK_X = -7 + ( (LLA_X/60) * (10) )
            L_IK_Y = 0.34 - ( (LLA_Z/-50) * (9) )
            L_IK_Z = 3 - ( (LLA_Y/-100) * 16 )
            
            self.L_IK_Target = [ L_IK_Z , L_IK_Y , L_IK_X ]
            
            R_IK_X = 6 + ( (RLA_X/60) * (10) )
            R_IK_Y = 0 - ( (RLA_Z/-35) * (9) )
            R_IK_Z = 3 - ( (RLA_Y/-100) * 16 )
            
            self.R_IK_Target = [ R_IK_Z , R_IK_Y , R_IK_X ]
            
            print([ int(RLA_X) , int(RLA_Z) , int(RLA_Y) ])
...
#                print ( [ f"X : {X} " , f"Y : {Y} " , f"Z : {Z}" ] )
            ...
            #print( round(self.max1,3) , round(self.min1,3) )
                ...
                self.Head_Z_Target = 0
                self.Head_X_Target = 0
                
                self.Chest_Y_Target = 0
                self.Chest_Z_Target = 0
                
                self.L_Shoulder_X_Target = 0
                self.R_Shoulder_X_Target = 0
                
                self.L_Upper_X_Target = 0
                self.L_Upper_Z_Target = 0
                
                self.R_Upper_X_Target = 0
                self.R_Upper_Z_Target = 0
                
                self.L_Lower_X_Target = 0
                self.L_Lower_Z_Target = 0
                
                self.R_Lower_X_Target = 0
                self.R_Lower_Z_Target = 0
                
                self.Hip_Y_Target = 0
                
                self.L_IK_Target = (0,0,0)
                self.R_IK_Target = (0,0,0)
        t8 = time.time()
        #print( f"Final : {t8-t7}" )
...
#        n
        ...
        L_IK_bone.location = ( (L_IK_bone.location[0]*0.8) + (self.L_IK_Target[0]*0.2) , (L_IK_bone.location[1]*0.8) + (self.L_IK_Target[1]*0.2) , (L_IK_bone.location[2]*0.8) + (self.L_IK_Target[2]*0.2))
        R_IK_bone.location = ( (R_IK_bone.location[0]*0.8) + (self.R_IK_Target[0]*0.2) , (R_IK_bone.location[1]*0.8) + (self.R_IK_Target[1]*0.2) , (R_IK_bone.location[2]*0.8) + (self.R_IK_Target[2]*0.2))
